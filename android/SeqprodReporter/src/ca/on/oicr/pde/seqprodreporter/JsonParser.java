				...
				try {
					...
					for (int i = 0; i < jsonRecords.length(); i++) {
						...
						Report newReport = new Report(tmp.getString("sample"),
								...
								tmp.getLong("crtime"), tmp.getLong("lmtime"),
								tmp.getString("wrun_id"),
								tmp.getString("status"), types[t], updated);
					...
					Log.e(ReporterActivity.TAG, "No JSON data for " + types[t]);
					e.printStackTrace();
				}
			}
		} catch (JSONException e) {
