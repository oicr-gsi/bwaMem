...
public class ReporterActivity extends ActionBarActivity implements
    ...
    private int updateFrequency; // in milliseconds
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//Read preferences
		this.sp = getPreferences(MODE_PRIVATE);
		this.updateHost = sp.getString("pref_hostName", null);
		this.updateRange = sp.getString("pref_summaryScope", null);
		String uf = sp.getString("pref_syncFreq", null);
		
		if (!uf.equals(R.string.pref_automaticUpdates_default)) {
			try {
	...
	public class SectionsPagerAdapter extends FragmentPagerAdapter {
		private List<RunListFragment> fragments;
		
		public SectionsPagerAdapter(FragmentManager fm) {
			super(fm);
			fragments = new ArrayList<RunListFragment>();
		}
		...
		public Fragment getItem(int position) {
			...
			if (position >= this.fragments.size() || null == this.fragments.get(position)) { 
			  fragments.add(position, RunListFragment.newInstance(position + 1));
			}
